        ...
        <div className="max-w-screen-xl mx-auto flex justify-between items-center px-4 py-4 md:px-8">
          ...
          <nav className="hidden md:flex gap-6 text-sm items-center">
            {navigation.map((item, idx) => (
              <Link key={idx} to={item.path} className="text-gray-700 hover:text-blue-600 font-medium">
                {item.title}
              </Link>
            ))}
            {isSignedIn ? (
              <>
                ...
                <SignOutButton>
                  <button className="px-4 py-2 bg-red-500 text-white rounded-full hover:bg-red-400 transition text-sm">
                    Cerrar sesión
                  </button>
                </SignOutButton>
              </>
            ) : (
              <Link to="/login" className="px-4 py-2 bg-blue-600 text-white rounded-full hover:bg-blue-500 transition">
                Iniciar Sesión
              </Link>
            )}
          </nav>
        ...
        {state && (
          <div className="md:hidden px-4 pb-4">
            <div className="space-y-2">
              {navigation.map((item, idx) => (
                <Link key={idx} to={item.path} className="block text-gray-700 hover:text-blue-600">
                  {item.title}
                </Link>
              ))}
              {isSignedIn ? (
                <>
                  <p className="text-sm text-blue-700 mt-2">Hola, {user.fullName || "usuario"}</p>
                  <SignOutButton>
                    <button className="mt-2 px-4 py-2 w-full bg-red-500 text-white rounded-full text-center text-sm hover:bg-red-400">
                      Cerrar sesión
                    </button>
                  </SignOutButton>
                </>
              ) : (
                <Link to="/login" className="block mt-2 px-4 py-2 bg-blue-600 text-white rounded-full text-center">
                  Iniciar Sesión
                </Link>
              )}
            </div>
